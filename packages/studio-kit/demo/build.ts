...
const SAMPLES = {
  metric: [
    ...
    },
    {
      ...
      note: "after the pricing change",
      trend: "down",
    },
    {
      variant: "badge",
      value: "3.2x",
      label: "faster renders",
      motion: "land",
    },
  ],
  lowerThird: [
  ...
  kpi: [
    {
      cells: [
        ...
        { label: "orders", value: "18,392" },
        { label: "refund rate", value: "0.8%", trend: "down" },
      ],
    },
    {
      variant: "row",
      cells: [
        { label: "before", value: "3.1s" },
        { label: "after", value: "0.4s", trend: "up" },
      ],
    },
  ],
  comparison: [
    {
      ...
      bValue: "2 days",
      winner: "b",
      note: "time to first user",
    },
    {
      variant: "versus",
      aLabel: "monolith",
      aValue: "99.2%",
      bLabel: "microservices",
      bValue: "97.1%",
      winner: "a",
    },
  ],
  ...
  steps: [
    {
      items: [
        { text: "Record the take" },
        { text: "Cut by transcript", note: "silence & retakes drop" },
        { text: "Ship it" },
      ],
    },
    {
  ...
  callout: [
    ...
    {
      variant: "quote",
      text: "Most points in a small space look good",
      support: "design principle 01",
    },
    {
      variant: "stamp",
      text: "VERIFIED",
      support: "reproduced on three machines",
    },
  ],
  code: [
    {
      ...
      file: "edit.ts",
      language: "typescript",
      code: "const project = await pireel.open();\nawait project.compose({ intent });",
    ...
    },
    {
      variant: "scroll",
      file: "pipeline.ts",
      code: Array.from({ length: 24 }, (_, i) =>
        i === 17
          ? "return dedupeFrames(frames);"
          : `const frame${i + 1} = prepare(source);`,
      ).join("\n"),
      highlightLine: 18,
    },
  ],
...
for (const [cid, samples] of Object.entries(SAMPLES)) {
  for (const props of samples) {
    for (const [theme, vars] of Object.entries(THEMES)) {
      const id = `d${n++}`;
      const { html, timeline } = render(cid, id, props, {
        box: { w: 640, h: 420 },
