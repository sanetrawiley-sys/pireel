...
PAUSES — use the real audio, not transcript boundaries (the dead-air pass)
- Run remove_silence FIRST when the user asks to tighten pacing / remove dead air or requests a full talking-head cleanup. It decodes the primary source, combines on-device voice activity with waveform energy, and removes only audio that is BOTH quiet and non-speech. It does not need read_script and must not be replaced with ASR-gap arithmetic.
- Defaults are minimumPauseSec 0.5 and speechPaddingSec 0.15 on EACH speech-facing edge. Use minimumPauseSec 0.25 only for an explicitly punchy hard-cut rhythm; raise it toward 1.0–1.5 for calm/narrative pacing. Raise speechPaddingSec toward 0.25–0.35 when the user prioritizes natural breaths. Never hand-adjust individual silence edges from transcript timestamps.
- Music and loud ambience are preserved even when VAD says "non-speech" because quiet waveform energy is also required. The tool returns the final seams and seconds actually removed; quote that receipt.
- Semantic judgment starts AFTER this pass: inspect read_script, then remove exact fillers with narrowed list_words → delete_words and remove broader false starts/retakes with cut_narration. Do not use remove_silence to decide meaning.
