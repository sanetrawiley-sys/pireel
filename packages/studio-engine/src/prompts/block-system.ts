...
LIGHTWEIGHT AD TYPOGRAPHY — when the instruction asks for a hook, benefit, attribute, proof qualifier, offer, reaction, feature pointer, CTA, flower text, or lightweight advertising typography over footage, this mode OVERRIDES the heavier structure and motion quotas below:
- This is still a Motion Graphic Component, not a separate text-sticker system. Render the instruction's exact approved phrase; do not expand it, paraphrase it, or invent a stronger claim. Build ONE short phrase with one dominant keyword or value. Use at most 1–2 small meaning-bearing devices: an underline, circle, pointer, tick, restrained burst, or local contrast patch. A type-only composition is valid; it does not need a chart, index rail, panel, icon, or multi-row structure.
...
MOTION — except for LIGHTWEIGHT AD TYPOGRAPHY above, use staged choreography rather than one fade. Compose entrance as STAGES on \`tl\` (total ≤1.2s unless beats say otherwise), then everything holds still:
...
- Genuine structure (≥1 non-text element), ONE focal element, size contrast ≥3× — not just words. LIGHTWEIGHT AD TYPOGRAPHY is the exception: one authored typographic hierarchy may be the complete structure.
- Motion is STAGED (≥2 stages, hero lands last) and uses ≥2 devices — not a single fade; then everything holds still. LIGHTWEIGHT AD TYPOGRAPHY instead follows its 0.18–0.35s decisive entrance and stable hold. With SPOKEN BEATS, no later content is visible before the beat that first mentions it. Selectors scoped, timing local.
