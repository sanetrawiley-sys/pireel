...
- Typography: line-height is unitless and explicit font-size always resolves to px. Because the platform inheritance baseline is already fixed in px, em may follow the text for letter-spacing, inline spacing, or a local decoration/icon; this is useful for authored display type. Keep composition-level geometry in px: padding 36–64px · gaps 16–40px · radius 16–28px · hairline 2px. Percentages are allowed only for relative placement and sizing. NEVER cm/mm/in/pt/pc/Q, rem/ch, vw/vh/vmin/vmax, container units, or responsive font formulas such as clamp()/calc(). Reduce CJK headline one tier vs latin.
...
CUSTOM DISPLAY TYPOGRAPHY — simple hooks, quotes, labels, benefits and CTAs belong in the native add_texts preset system and should not reach this generator. When an instruction deliberately reaches this path, its meaning must require a custom composed mark, annotation or multi-object typographic relationship beyond those presets; this mode then OVERRIDES the heavier structure and motion quotas below:
- This is a Motion Graphic Component. Render the instruction's exact approved phrase; do not expand it, paraphrase it, or invent a stronger claim. Build ONE short phrase with one dominant keyword or value and only the meaning-bearing devices the brief requires: for example an underline, circle, pointer, tick, restrained burst, or local contrast patch. It does not need a chart, index rail, panel, icon, or multi-row structure.
- Root and wrapper stay transparent. Default to NO filled card, rounded panel, box-in-box, dashboard, multi-card explainer, or full-screen title plate. A compact local backing shape is allowed only when the BACKDROP makes direct type unreadable; it must hug the phrase rather than become a generic card.
- Keep the component spatially compact and anchor it to the BACKDROP's real negative space. Do not cover the subject, product, hands, face, burned-in copy, managed-caption band, or platform UI safe zones.
- Entrance is 0.18–0.35s and may be a single decisive reveal or two tightly coupled beats; then hold completely still. Optional exit is short. No loop, ambient drift, multi-stage presentation choreography, or decorative stagger for a simple phrase.
- Author the inner composition upright. Whole-object position, proportional scale, and rotation belong to the Motion card's canvas transform controls; never bake arbitrary alternating tilt into inner CSS.
...
MOTION — except for CUSTOM DISPLAY TYPOGRAPHY above, use staged choreography rather than one fade. Compose entrance as STAGES on \`tl\` (total ≤1.2s unless beats say otherwise), then everything holds still:
...
- SPOKEN-CONTENT REVEALS: when SPOKEN BEATS are given, map EVERY independently spoken content item to the beat that first mentions it and reveal/highlight it at that exact LOCAL time. Content belonging to a later beat MUST stay hidden before it is spoken; never put the component's complete final state on screen at time 0. This applies to comparison sides, rows, chart series/annotations, process nodes and supporting claims — not only things explicitly called a list. A persistent heading, frame or already-spoken item may enter near the start and remain settled/dimmed. The active Frame decides the visual motion grammar, but it never overrides this speech synchronization.
- EMPHASIS BEATS: land the hero exactly on its beat, and give later beats a subtle re-emphasis (accent pulse scale 1→1.06→1, or the next row lighting up) so the fragment stays ALIVE while held on screen — but no loops, no drifting after the last beat.
...
- Every text role either inherits Studio's ${BLOCK_TEXT_BASELINE_PX}px fallback or uses an explicit px/\`--type-*\` size of at least ${BLOCK_MIN_READABLE_FONT_PX}px; no browser-default 16px text, unresolved typography, or \`font\` shorthand.
- Type and composition-level geometry use px in the real authored box/canvas coordinate system supplied by the prompt; local type spacing and decorations may use em only after that px type baseline exists. Layout adapts to the box's aspect ratio; NO vw/vh, NO cqmin fonts.
...
- Genuine structure (≥1 non-text element), ONE focal element, size contrast ≥3× — not just words. CUSTOM DISPLAY TYPOGRAPHY is the exception: one authored typographic hierarchy plus its requested meaning-bearing mark may be the complete structure.
- Motion is STAGED (≥2 stages, hero lands last) and uses ≥2 devices — not a single fade; then everything holds still. CUSTOM DISPLAY TYPOGRAPHY instead follows its 0.18–0.35s decisive entrance and stable hold. With SPOKEN BEATS, no later content is visible before the beat that first mentions it. Selectors scoped, timing local.
