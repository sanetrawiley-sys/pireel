...
import { components, isComponentId, type ComponentDef, type ComponentId } from '@pireel/studio-kit';
...
const SEARCH_STOP_WORDS = new Set(['a', 'an', 'and', 'as', 'at', 'card', 'component', 'for', 'graphic', 'in', 'it', 'of', 'on', 'one', 'or', 'show', 'the', 'this', 'to', 'with']);
...
function englishWords(value: string): Set<string> {
  return new Set(normalized(value).split(' ').filter((word) => /^[a-z0-9]+$/.test(word) && word.length > 1 && !SEARCH_STOP_WORDS.has(word)));
}

function retrievalScore(id: ComponentId, definition: ComponentDef, query: string): number {
  const queryNormalized = normalized(query);
  const queryCompact = queryNormalized.replace(/\s+/g, '');
  const queryWords = englishWords(query);
  let score = 0;
  if (new RegExp(`(?:^|[^a-z0-9])${id.toLocaleLowerCase()}(?:$|[^a-z0-9])`, 'i').test(queryNormalized)) score += 100;
  for (const term of definition.searchTerms) {
    ...
    const containsCjk = /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]/u.test(termNormalized);
    if (containsCjk ? queryCompact.includes(termCompact) : queryNormalized.includes(termNormalized)) {
...
function structuralCandidates(query: string): ComponentId[] {
  ...
  add('comparison', /(?:\bvs\.?\b|versus|compare|comparison|before\s*(?:and|&|\/)?\s*after|对比|比较|优劣|前后)/i);
  add('chart', /(?:chart|graph|bar|column|donut|ranking|trend|图表|柱状|条形|环形|排名|趋势)/i);
  add('steps', /(?:steps?|process|workflow|timeline|roadmap|sequence|步骤|流程|时间轴|路线图|顺序)/i);
  add('lowerThird', /(?:lower\s*third|speaker|byline|job\s*title|姓名|人名|人物|身份|职位|署名)/i);
  add('title', /(?:opener|chapter|section|closer|outro|headline|\bcta\b|开场|章节|标题页|结尾|片尾|行动号召)/i);
  add('callout', /(?:quote|keyword|verdict|insight|warning|punchline|金句|关键词|结论|洞察|警告|强调)/i);
  add('kpi', /(?:\bkpi\b|dashboard|multiple\s+(?:metrics|numbers)|指标组|多指标|多个数字|数据组|仪表盘)/i);
  add('metric', /(?:\d+(?:\.\d+)?\s*(?:%|％|x|倍|万|亿)|percentage|rate|score|百分比|比率|增长率|转化率|完成率|指标|大数字)/i);
  return out;
}
...
export function retrieveComponentCandidates(input: ComponentRetrievalInput): ComponentId[] {
  const limit = Math.min(Math.max(Math.round(input.limit ?? MAX_COMPONENT_CANDIDATES), 1), MAX_COMPONENT_CANDIDATES);
  ...
  for (const id of allowed) {
    if (new RegExp(`(?:^|[^a-z0-9])${id.toLocaleLowerCase()}(?:$|[^a-z0-9])`, 'i').test(query)) add(id);
  }
