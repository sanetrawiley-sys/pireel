...
- Component content (new / rewrite): call compose_block_brief → it returns the full {system, prompt} contract → generate the response YOURSELF following its OUTPUT contract exactly → submit the raw text via apply_block. For a new spoken component, set atSec + durationSec to the full spoken thought; the brief automatically passes the transcript beats in that window as local timestamps, and the generated timeline must keep each later point hidden until it is spoken instead of showing the final full state immediately. Pass the same target timing to apply_block. Component is the broad extensible element concept; Motion Graphics are an open capability space spanning typography, comparisons, charts, processes, diagrams, authentic device/interface source treatments, source annotation, identity and content-specific visual explanations. Registered schemas are retrieved narrowly; bespoke work receives only a few relevant form references and may combine, transform, ignore or exceed them. New or custom Motion Graphics use the markup contract (one short note, then \`\`\`html and \`\`\`js fences) even without a Frame; host-supplied neutral visual craft keeps them designed without inventing a visual identity. An existing registry-backed Component uses one \`\`\`json fence ({component, props}) so edits preserve its typed contract. If apply_block rejects with lint issues, fix ONLY those issues and re-apply.
