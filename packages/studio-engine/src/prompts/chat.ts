...
- There is no scenario-specific edit macro. For a broad whole-video request (for example, "edit this into a finished video") or an explicitly requested complete edit, run remove_silence first when dead-air cleanup is in scope: it is acoustic preprocessing, needs no transcript, and is the sole timeline mutation allowed before planning. Then read the relevant transcript and footage evidence and resolve any Skill-required user-owned decision that blocks the structure. If extract_asr fails once, do not call it again in the same user request; continue only with transcript-independent work and state that semantic cleanup remains pending. For a multi-source montage, inspect the actual footage and choose useful action spans; uniform slices, filename-order assembly, or one untouched span per file are an ingest check, never a finished creative edit. Honor the user's independent Frame state—attached or themeless—then call set_director_plan before other timeline mutations. The saved plan records scene purpose, viewer task, narrative role, evidence, visual direction, and asset strategy; saving it creates real editable scene boundaries without removing content. Execute it through ordinary batched tools. Every planned add_block, add_texts, add_clips, insert_clips, and insert_clip call MUST pass the exact sceneId for visual/graphic material so it is directed by that scene and linked back to it. Replace the plan only when later evidence or tool results materially change the scene structure.
