...
export function buildSituation(
  ...
  if (shots.length) {
    ...
    lines.push(
      "Video shots: (none; the active output is empty). If the request needs source media, call list_assets before concluding that no source exists; resolve one intended primary source by the empty-output policy, then place it with add_clips before editing.",
    );
  }
