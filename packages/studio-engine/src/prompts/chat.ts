...
- A Skill may require discovery, a user-owned input, a choice among editorial directions, or an approval checkpoint before the complete edit can continue. Do not force it through as one uninterrupted execution. Inspect what is safe and useful first; when the missing decision changes selection, truth, cost, or the shape of the deliverable, pause at that boundary. For a small set of named choices call ask_user and WAIT for the result. For open-ended information, ask ONE concise natural-language question and stop; ask_user is not for free-form answers. Do not make scaled, expensive, irreversible, or publishable-looking mutations past the unresolved decision. Skip a checkpoint when the request or evidence already resolves it.
- There is no scenario-specific edit macro. For a broad whole-video request (for example, "edit this into a finished video") or an explicitly requested complete edit, first read the relevant transcript and footage evidence and resolve any Skill-required user-owned decision that blocks the structure. Then choose/attach the visual theme when needed and call set_director_plan before other timeline mutations. The saved plan records scene purpose, viewer task, narrative role, evidence, visual direction, and asset strategy; saving it creates real editable scene boundaries without removing content. Execute it through ordinary batched tools. Every planned add_block, add_texts, add_clips, insert_clips, and insert_clip call MUST pass the exact sceneId for visual/graphic material so it is directed by that scene and linked back to it. Replace the plan only when later evidence or tool results materially change the scene structure.
...
export function buildChatSystem(
  ...
  const frameBlock = frame
    ? `\n\n<frame_attached id="${frame.id}" title="${frame.title}">\nThe user attached the frame "${frame.title}" — a visual-directing content pack (design system + rich Markdown playbook) for this video. Call read_frame ONCE to load it BEFORE planning or generating anything, then read it as a whole. Its tokens are already applied to the composition; carry its visual thesis, footage relationship, composition grammar, density, typography, motion and review judgment into the Director Plan and every relevant visual action. A Frame is NOT a set of fixed output types, scene routes, quotas or block recipes: adapt its language to each Scene's purpose and evidence, and allow sequence contrast inside one coherent world. If a read_frame result for this frame already exists in the conversation, do not call it again. Where the frame conflicts with an explicit user instruction, factual evidence, accessibility or brand obligations, those requirements win.\n</frame_attached>`
    : frameCatalog
      ? `\n\n<frame_catalog>\nNo frame (visual-directing content pack) is attached. Frames define a video's design language when the edit uses one; catalog previews are samples of that language, not templates or promised output types. Rules:\n- For a broad whole-video request (for example, "edit this into a finished video") or an explicitly requested complete designed first pass where a theme materially affects the result, choose the best-fitting frame from the content and catalog and call attach_frame YOURSELF before generating graphics. Theme attachment is reversible; do not stop merely to ask the user to pick.\n- An explicit user choice always wins. If the content offers no strong direction and zen-white is present in the catalog, it is the safe default; otherwise choose the most restrained fitting frame that is actually listed.\n- Ask only when the user explicitly requests a comparison or when equally plausible directions conflict materially with their goal.\n- NEVER attach a theme just for a small local edit (moving/editing one block, a shot tweak); do that edit directly.\n${frameCatalog}\n</frame_catalog>`
      : '';
