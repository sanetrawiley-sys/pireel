...
export function buildMcpTools(): McpToolDef[] {
  ...
  out.push(
    ...
    },
    {
      name: 'import_media',
      description:
        "Import LOCAL files into Pireel. TWO STEPS: ① call with NO arguments → returns a short-lived import `token` (30 min) plus the exact connected `base_url`; ② run the plugin's import helper (skills/pireel/scripts/import-media.mjs) with `--base <base_url> --token <token>` and the file paths. Main video, B-roll (--broll), image, and audio bytes stream straight into the OPEN studio tab and remain in device-local OPFS (no cloud upload). Open the Studio handoff in YOUR OWN built-in/embedded browser first — on Codex, use the in-app Browser runtime and do not proactively choose connected Chrome. If and only if the helper explicitly returns `local loopback is unreachable from this browser`, release that isolated tab, open one fresh handoff in a controllable connected browser that shares the agent host's 127.0.0.1, and retry once with a fresh token. Never replace this with a cloud upload or hidden file-input automation. The helper returns local registrations; pass standalone audio registration unchanged to register_media, then place it with add_clips using narration/music/sfx from the user's intent.",
      inputSchema: {
        type: 'object',
        additionalProperties: false,
        properties: {
